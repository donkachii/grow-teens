...
async function uniqueCourseSlug(title: string, { excludeId }: { excludeId?: number } = {}): Promise<string> {
...
export const getCourses = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (difficulty) where.difficulty = difficulty as Prisma.EnumDifficultyLevelNullableFilter;
    if (search) {
      where.OR = [
    ...
    const validSortFields = ["createdAt", "title", "difficulty"] as const;
    const sortField = validSortFields.includes(sortBy as typeof validSortFields[number]) ? sortBy : "createdAt";
...
export const getCourseById = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(500).json({ error: "Failed to retrieve course", details: (error as Error).message });
  }
};
...
export const updateCourse = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (categoryId !== undefined) data.category = { connect: { id: parseInt(categoryId) } };
    if (difficulty !== undefined) data.difficulty = difficulty;
    if (instructorId !== undefined) data.instructor = { connect: { id: parseInt(instructorId) } };
    ...
    if ((error as PrismaError).code === "P2025") return res.status(404).json({ error: "Course not found" });
...
export const getAdminCourses = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(500).json({ error: "Failed to fetch courses", details: (error as Error).message });
  }
};
...
export const updateModule = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if ((error as PrismaError).code === "P2025") return res.status(404).json({ error: "Module not found" });
    res.status(500).json({ error: (error as Error).message });
  }
};
...
export const getModules = async (req: Request, res: Response) => {
  const { courseId, page = "1", limit = "10" } = req.query as Record<string, string>;

  try {
    const skip = (parseInt(page) - 1) * parseInt(limit);
    const where: Prisma.CourseModuleWhereInput = courseId ? { courseId: parseInt(courseId) } : {};
...
export const createContentUnit = async (req: Request, res: Response) => {
  const {
    title,
    description,
    moduleId,
...
export const getContentUnitsByModule = async (req: Request, res: Response) => {
...
export const getContentUnitById = async (req: Request, res: Response) => {
  const { id } = req.params;
  try {
...
export const updateContentUnit = async (req: Request, res: Response) => {
  const { id } = req.params;
  const { title, description, orderNumber, contentType, content, estimatedMinutes, isRequired } = req.body;

  try {
    const data: Prisma.ContentUnitUpdateInput = {};
    if (title !== undefined) data.title = title;
    if (description !== undefined) data.description = description;
    if (orderNumber !== undefined) data.orderNumber = parseInt(orderNumber);
    ...
    if ((error as PrismaError).code === "P2025") return res.status(404).json({ error: "Content unit not found" });
    res.status(500).json({ error: "Failed to update content unit" });
  }
};

export const deleteContentUnit = async (req: Request, res: Response) => {
  const { id } = req.params;
  try {
...
export const enrollInCourse = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(500).json({ error: "Failed to enroll", details: (error as Error).message });
  }
};
...
export const getEnrolledCourses = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(500).json({ error: "Failed to retrieve enrolled courses", details: (error as Error).message });
  }
};
...
export const updateUnitProgress = async (req: Request, res: Response) => {
...
async function updateModuleAndCourseProgress(enrollmentId: number, courseId: number): Promise<void> {
  ...
  for (const module of modules) {
    ...
    const enrollmentRecord = await prisma.enrollment.findUnique({ where: { id: enrollmentId } });
    if (!enrollmentRecord) continue;
