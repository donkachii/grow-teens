...
async function uniqueProgramSlugFromTitle(title: string, { excludeId = undefined }: { excludeId?: number } = {}): Promise<string> {
...
function flattenModules(program: any) {
...
function serializeProgram(program: any, { withModules = false } = {}) {
...
export const addCourseToProgram = async (req: Request, res: Response) => {
  const { id } = req.params;
...
export const removeCourseFromProgram = async (req: Request, res: Response) => {
